...
$wp_tests_dir = './tests/tmp/wordpress-tests-lib';
validate_file_exits( "{$wp_tests_dir}/includes/functions.php" );

$wp_core_dir    =  './tests/tmp/wordpress';
...
function load_plugins() {
	...
	require $plugin_dir . '/storepress-base-plugin.php';
}
...
function validate_file_exits( string $file_name ) {
	if ( ! file_exists( $file_name ) ) {
		echo "Could not find {$file_name}, have you run bin/install-wp-tests-with-wc.sh ?" . PHP_EOL; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		exit( 1 );
	}
}
