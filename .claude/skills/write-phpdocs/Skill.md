...
- Class DocBlock: `@name`, description, `@method` — never use `@extend`
- Ensure to add `declare( strict_types=1 );` after file level doc block.

## Methods

- DocBlock with `@param`, `@return`, and `@since`
- Use `git log -S "method_name"` to find in which version it was introduced and add `@since` annotation with that version
...
- Use `git log -S "hook_name"` to find in which version it was introduced and add `@since` annotation with that version
